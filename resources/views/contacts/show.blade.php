<x-app-layout>
                                ...
                                <div class="flex flex-wrap -m-2">
                                    ...
                                    <div class="p-2 w-full">
                                        <div class="relative">
                                            ...
                                            <div class="w-full border border-gray-300 rounded text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">{{ $contact->name }}</div>
                                        </div>
                                    </div>
                                   ...
                                   <!-- 件名  -->
                                    <div class="p-2 w-full">
                                        <div class="relative">
                                            ...
                                            <div class="w-full border border-gray-300 rounded focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">{{ $contact->title }}</div>
                                        </div>
                                    </div>
                                   ...
                                   <div class="p-2 w-full">
                                    ...
                                    <!-- 削除ボタン  -->
                                    <form id="delete_{{ $contact->id }}" class="mt-20" method="post" action="{{ route('contacts.destroy', ['id' => $contact->id] )}}">
                                    @csrf
                                    <div class="p-2 w-full">
                                        <a hred="#" data-id="{{ $contact->id }}" onclick="deletePost(this)" class="flex mx-auto text-white bg-pink-500 border-0 py-2 px-8 focus:outline-none hover:bg-pink-600 rounded text-lg">削除</a>
                                    </div>
                                    </form>
    ...
    <!--確認メッセージ-->
    <script>
        function deletePost(e){ //onclick="deletePost(this)"の情報 = e
            'use strict'
            if(confirm('本当に削除をしますか？')){
                document.getElementById('delete_' + e.dataset.id).submit() //data-id="{{ $contact->id }}"
            }
        }
    </script>
</x-app-layout>
